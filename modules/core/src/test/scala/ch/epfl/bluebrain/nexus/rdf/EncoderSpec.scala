...
class EncoderSpec extends RdfSpec {
  ...
  "An Encoder" should {
    ...
    "correctly encode a one element Set" in {
    ...
    "correctly encode an empty Set" in {
      val g = Encoder[Set[IntValue]].apply(Set.empty)
      g.triples.isEmpty shouldEqual true
    ...
    "correctly encode sequences" when {
